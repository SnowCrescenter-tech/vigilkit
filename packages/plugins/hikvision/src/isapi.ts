...
/**
 * A minimal, dependency-free XML element model sufficient for ISAPI responses.
 * Hikvision ISAPI endpoints return flat or shallow XML documents.
 */
...
/**
 * Tolerant XML parser for ISAPI payloads. Handles the XML declaration,
...
export function parseXml(xml: string): XmlElement {
  ...
  if (root === null || root.next === -1) {
    throw new HikvisionError('PARSE', 'Failed to parse XML: no root element');
  }
  ...
  if (node === null || node.kind !== 'element') {
    throw new HikvisionError('PARSE', 'Failed to parse XML: no root element found');
  }
  return node.element;
}

 ...
 * Throws `HikvisionError('AUTH')` if the header is missing, is not Digest, or
...
export function parseDigestChallenge(header: string | null | undefined): DigestChallenge {
  if (!header || !/^Digest\s+/i.test(header.trim())) {
    throw new HikvisionError('AUTH', 'Server did not issue a Digest WWW-Authenticate challenge');
  }
  ...
  if (!realm || !nonce) {
    throw new HikvisionError('AUTH', 'Digest challenge missing realm or nonce');
  }
